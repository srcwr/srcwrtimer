...
public:
	...
	ClosestPosObject(std::vector<Point> *_data, int _startidx)
	{
		startidx = _startidx;
		data = _data;
		container = nanoflann_shim_create_container(data->data(), data->size());
	}

	~ClosestPosObject()
	{
		nanoflann_shim_delete_container(container);
		delete data;
	}
};
...
static cell_t N_ClosestPos_ClosestPos(IPluginContext* ctx, const cell_t* params)
{
	...
	ClosestPosObject *closestpos = new ClosestPosObject(data, startidx);
...
{
	ClosestPosObject* closestpos;
	HandleError err;
	HandleSecurity sec(ctx->GetIdentity(), myself->GetIdentity());

	if ((err = handlesys->ReadHandle(params[1], g_ClosestPosType, &sec, (void **)&closestpos))
		!= HandleError_None)
	{
	...
	auto ret_index = nanoflann_shim_get_nearest(closestpos->container, query_pt);

	return closestpos->startidx + ret_index;
}
